...
mttkrp.Tensor <- function(x, U, mode, ...) {
  ...
  if (inherits(U, "KTensor")) {
    U_list <- lapply(U$U, function(mat) mat)
  ...
  for (i in other_modes) {
    ...
    if (is.null(R)) {
      ...
      stop("All factor matrices except the skipped mode must have the same number of columns.")
    }
    U[[i]] <- Ui
  }

  if (is.null(R)) {
    stop("No factor matrices available to form the Khatri-Rao product.")
  }

  V <- matrix(0, nrow = x$dim()[mode], ncol = R)
  for (r in seq_len(R)) {
...
mask.Tensor <- function(x, w, ...) {
  ...
  w_subs <- find(tensor(w$data, dims = w_dims))
  if (nrow(w_subs) == 0) {
    return(numeric(0))
  }
