...
test_that("ttm multiple dispatch handles negative modes", {
  # 4D tensor: 5x4x3x2
  t4d <- tensor(array(1:120, dim = c(5, 4, 3, 2)))

  # We want to contract all dimensions EXCEPT mode 3 (size 3)
  # So we contract modes 1 (size 5), 2 (size 4), 4 (size 2)
  m1 <- matrix(1:15, nrow = 3, ncol = 5) # 3x5 for mode 1 (size 5)
  m2 <- matrix(1:8, nrow = 2, ncol = 4) # 2x4 for mode 2 (size 4)
  m4 <- matrix(1:2, nrow = 1, ncol = 2) # 1x2 for mode 4 (size 2)

  # mode = -3 means modes c(1, 2, 4)
  result <- ttm(t4d, list(m1, m2, m4), mode = -3)

  # Resulting dimensions: 3 (from m1), 2 (from m2), 3 (tensor mode 3 left alone), 1 (from m4)
  expected_dims <- c(3, 2, 3, 1)
  expect_equal(dim(result$as_array()), expected_dims)
})

test_that("ttm multiple dispatch handles list subsetting", {
  # 4D tensor: 5x4x3x2
  t4d <- tensor(array(1:120, dim = c(5, 4, 3, 2)))

  # Provide a list of 4 matrices
  m1 <- matrix(1:15, nrow = 3, ncol = 5)
  m2 <- matrix(1:8, nrow = 2, ncol = 4)
  m3 <- matrix(1:6, nrow = 2, ncol = 3)
  m4 <- matrix(1:2, nrow = 1, ncol = 2)
  full_list <- list(m1, m2, m3, m4)

  # Select modes 1 and 3 from the list of 4
  # list[c(1, 3)] selects m1 and m3 automatically
  result <- ttm(t4d, full_list, mode = c(1, 3))
